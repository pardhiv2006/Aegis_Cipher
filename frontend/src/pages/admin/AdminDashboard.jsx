    ...
    <div className="flex flex-col gap-10">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6">
        {loading ? [...Array(6)].map((_, i) => (
          ...
          <motion.div 
            ...
            className="glass-card p-6 flex flex-col items-center justify-center text-center group hover:shadow-2xl hover:-translate-y-2 transition-all border-white/80 bg-white/40"
          >
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-5 bg-white border border-slate-100 shadow-sm group-hover:scale-110 transition-transform`}>
            ...
            <p className="text-2xl font-black text-slate-900 tracking-tighter">{c.value}</p>
          </motion.div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-8 flex flex-col gap-10">
           ...
           <div className="glass-card p-10 bg-white/60 border-white/80 shadow-2xl flex flex-col relative overflow-hidden">
              ...
              <div className="flex items-center justify-between mb-12 relative z-10">
                 <div className="flex items-center gap-5">
                    ...
                    <div>
                       <h3 className="text-lg font-black text-slate-900 tracking-tighter leading-none mb-1.5 uppercase">Security Access Matrix</h3>
                       <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Master Identity Flow</p>
                    </div>
                 </div>
                 <div className="flex items-center gap-8">
                    <div className="flex items-center gap-2.5">
                       <div className="w-2.5 h-2.5 rounded-full bg-emerald-500 shadow-[0_0_8px_#10B981]" />
                       <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Authorized</span>
                    </div>
                    <div className="flex items-center gap-2.5">
                       <div className="w-2.5 h-2.5 rounded-full bg-rose-500 shadow-[0_0_8px_#F43F5E]" />
                       <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Restricted</span>
                    </div>
                 </div>
              </div>
              
              <div className="flex-1 flex items-end gap-6 h-80 px-4 pb-4 relative z-10">
                 {[65, 85, 50, 95, 75, 60, 80, 45, 90, 70, 98, 55].map((h, i) => (
                    <div key={i} className="flex-1 flex flex-col items-center gap-5 group h-full justify-end">
                       <div className="w-full relative flex items-end gap-1.5 h-full">
                          <motion.div 
                            initial={{ height: 0 }}
                            animate={{ height: `${h}%` }}
                            className="flex-1 bg-slate-900 rounded-t-xl transition-all group-hover:bg-indigo-600 shadow-xl"
                          />
                          <motion.div 
                            initial={{ height: 0 }}
                            animate={{ height: `${Math.max(15, 100-h)}%` }}
                            className="w-3 bg-rose-500/10 rounded-t-xl group-hover:bg-rose-500/40 transition-all"
                          />
                       </div>
                       <span className="text-[9px] font-black text-slate-400 uppercase tracking-tighter group-hover:text-slate-900 transition-colors">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div className="glass-card p-10 bg-slate-900 text-white rounded-[40px] shadow-2xl shadow-slate-200">
                 <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.4em] mb-6 flex items-center gap-3">
                    <Shield className="w-5 h-5" /> Master Audit Overview
                 </h4>
              ...
              <div className="glass-card p-10 bg-white/60 rounded-[40px] border-white/80">
                 <h4 className="text-[10px] font-black text-rose-500 uppercase tracking-[0.4em] mb-6 flex items-center gap-3">
                    <Terminal className="w-5 h-5" /> System Integrity
                 </h4>
                 <p className="text-sm text-slate-500 leading-relaxed font-medium">
                    Our ABE protocols are currently operating at a peak performance level. All identity attributes are being verified against the central master registry in real-time, ensuring that the 101 secure resources remain protected by the defined departmental and role-based policies.
                 </p>
              </div>
           </div>
        </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-10">
           ...
           <div className="glass-card flex-1 border-white/80 shadow-2xl overflow-hidden flex flex-col bg-white/40 p-10">
              <div className="mb-12">
                 <h3 className="text-sm font-black text-slate-900 uppercase tracking-[0.2em] leading-none mb-2.5">Security Posture</h3>
                 ...
                 {[
                    ...
                    >
                       <div className="flex items-center gap-4">
                          <div className={`w-10 h-10 rounded-xl bg-${item.color}-50 flex items-center justify-center group-hover:scale-110 transition-transform`}>
                             <item.icon className={`w-5 h-5 text-${item.color}-500`} />
                          </div>
                          <span className="text-[11px] font-black text-slate-600 uppercase tracking-tight">{item.label}</span>
                       </div>
                       <span className={`text-[10px] font-black text-${item.color}-600 uppercase tracking-widest`}>{item.value}</span>
                    </motion.div>
                 ))}
              </div>

              <div className="mt-12 p-6 rounded-[32px] bg-slate-900 text-white relative overflow-hidden">
                 <div className="absolute top-0 right-0 w-24 h-24 bg-white/10 blur-3xl" />
                 <p className="text-[9px] font-black uppercase tracking-[0.3em] mb-3 opacity-50">Admin Notice</p>
           ...
           {/* Health Card */}
           <div className="glass-card p-8 bg-indigo-600 text-white rounded-[40px] shadow-2xl shadow-indigo-100 relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-white blur-[80px] opacity-20" />
              <div className="flex items-center gap-4 mb-6">
                 <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center">
                    <Zap className="w-5 h-5 text-white" />
                 </div>
                 <h3 className="text-[10px] font-black uppercase tracking-[0.3em]">Quantum Node Health</h3>
              </div>
